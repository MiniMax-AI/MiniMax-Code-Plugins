...
> is open and waiting for review. **To participate in the beta**: install the standalone
> single-plugin repo ([`antianqi/comfyui-studio`](https://github.com/antianqi/comfyui-studio))
> following the instructions in [`BETA.md`](BETA.md), then report issues back so we can
> fix them before the official merge.
