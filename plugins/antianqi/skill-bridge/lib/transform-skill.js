...
export async function transformSkill({ inputPath, report, classify, outDir }) {
  ...
  const skillOut = path.join(outDir, 'SKILL.md');
  await fs.writeFile(skillOut, skillText, 'utf-8');
  written.push(skillOut);

  for (const ref of references) {
    const refPath = path.join(outDir, 'references', ref.file);
  ...
  const reportPath = path.join(outDir, 'conversion-report.md');
