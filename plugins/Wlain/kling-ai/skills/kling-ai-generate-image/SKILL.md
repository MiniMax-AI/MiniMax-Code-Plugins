...
description: 通过 MiniMax Code 中的 Kling AI 生成高质量图像。支持文生图、图生图，适用于海报、产品摄影、广告创意和参考素材创作。
---
...
9. 优先使用 MiniMax Code 自动呈现的原生工具结果。不要由 Skill 手写 Markdown 图片/视频语法、额外媒体附件或缩略图、重复下载链接；没有原生媒体呈现时只返回至多一个主图链接。将 `generationId` 显示为**任务编号**，除非故障排查需要，否则不对外显示 `taskTraceId`。
