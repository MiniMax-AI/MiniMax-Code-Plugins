...
export class Engine extends EventEmitter {
 ...
 async stop(id,intent='cancelled'){const ctx=this.active.get(id);if(!ctx){const run=this.store.get(id);if(run?.status==='pending_review'&&intent==='cancelled'){run.status='cancelled';this.save(run);this.emitEvent(id,'run.finished',{status:'cancelled'});}return this.snapshot(id);}ctx.intent=intent;ctx.reason=intent==='paused'?'用户暂停，已完成结果可复用':'用户取消';ctx.run.status=intent==='paused'?'pausing':'stopping';this.save(ctx.run);this.emitEvent(id,'run.stopping',{intent});ctx.controller.abort();void ctx.finish(false,ctx.reason);await ctx.done;return this.snapshot(id);}
