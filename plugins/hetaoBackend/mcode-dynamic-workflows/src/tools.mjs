...
export function createToolHandler(engine,getURL){return async(name,args={})=>{
 ...
 case 'workflow_status':if(args.runId)return summary(engine.snapshot(args.runId));return {runs:engine.store.list().map(r=>summary(r)),integrityHeads:engine.store.integrityHeads(),...(args.verifyIntegrity===true?{integrity:engine.store.verifyIntegrity()}:{})};
