...
export async function mcodeExecute(spec,{signal,onEvent,workspace,command,args=[],configPath,timeoutMs,maxSteps}) {
  ...
  return new Promise((resolve,reject)=>{
    ...
    let buffer='',stderr='',terminal=null,protocolError=null,finished=false,killTimer,watchdogExpired=false,stopping=false;
    const stop=()=>{if(stopping||finished)return;stopping=true;child.kill('SIGTERM');killTimer=setTimeout(()=>child.kill('SIGKILL'),2000);killTimer.unref();};
    ...
    child.stdout.on('data',chunk=>{buffer+=chunk;if(buffer.length>2_000_000){protocolError=new Error('Agent 输出超过协议缓冲上限');stop();return;}let i;while((i=buffer.indexOf('\n'))>=0){line(buffer.slice(0,i));buffer=buffer.slice(i+1);}});
    child.stderr.on('data',chunk=>{stderr=(stderr+chunk).slice(-2000);});
    function settle(error,value){if(finished)return;finished=true;clearTimeout(timer);clearTimeout(killTimer);signal.removeEventListener('abort',stop);error?reject(error):resolve(value);}
    child.on('error',e=>settle(failureError({code:'MCODE_START_FAILED',message:`无法启动 MCode（${e.code??'unknown'}）：${safeDetail(e.message)}`,suggestion:'确认已安装 mcode，命令可用，工作区路径存在。'})));child.on('close',(code,exitSignal)=>{
